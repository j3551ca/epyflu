...
def parse_gisaid_jsons(gisaid_json_dict: dict[str,tuple[str, str]]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    meta_minimal = meta[["isolate_name", "collection_date", "subtype", 
                        "location", "host"]]
    meta_melted = pd.melt(meta, id_vars="isolate_name", value_vars=seg_cols, 
                        var_name="segment", value_name="segment_seq_id")

    #only what has been uploaded is deposited into db
    epi_isl_meta = pd.merge(epi_isl_df, meta_minimal, 
    how = "left", left_on = "sample_id", right_on = "isolate_name").drop(columns = ["isolate_name"])
    ...
    epi_segs_meta = pd.merge(epi_segs_df, meta_melted,
                            how="left",
                            left_on = "sample_id",
                            right_on= "segment_seq_id").drop(columns = ["segment_seq_id"])
                            #  validate = "one_to_one")

    epi_segs_meta.rename(columns = {"sample_id" : "seg_id",
    "isolate_name": "isolate_id"}, inplace = True) 
...
def gisaid_search(user: str, password: str, isl_ids: str) -> pd.DataFrame:
    ...
    gisaid_df = gisflu.search(cred, 
            searchPattern=isl_ids,
            submitDateFrom="2017-01-01",
            recordLimit=700000)
