...
from sqlite_db import (parse_gisaid_jsons, add_to_sqlite_db, collect_unreleased, 
gisaid_search, update_release_status)
...
def download_vars(args, var_source):
    ...
    if not args.segments and var_source == "prompted": #assume interactive mode
        seglist = input("Please enter list of segments separated by commas to download data for or press Enter for <default>. <HA,NA>,PB1,PB2,PA,NP,MP,NS: ")
        if seglist:
            seg_list=seglist
    seg_list = seg_list.split(',')
    seg_list = [item.strip() for item in seg_list]
    if not args.download_type and var_source == "prompted":
        dtype = input("Please enter type of data to download or press Enter for <default>. <'metadata'>,'dna','protein': ")
        if dtype:
            download_type = dtype
    if not args.gisaid_ids:
        gids = input("Please enter list of GISAID IDs separated by commas to download data for. ex. EPI_ISL_1,EPI_ISL_2,EPI_ISL_45: ") 
    gids = gids.split(',')
...
def upload_vars(args, var_source):
    ...
    if not args.dateformat and not args.input and var_source == "prompted":
        dateformat = input("Please enter format of dates or press Enter for <default>. <YYYYMMDD>,YYYYDDMM,DDMMYYYY,MMDDYYYY: ")
        if dateformat:
            dateform=dateformat
...
def main(args):
    ...
    if args.command is None:
        ...
        if args.command == "upload":
            ...
            gisaid_jsons = gisaid_upload(verified_datasets, usr, psswd, cid, 
                    dateform, log_dir)
        ...
        if args.command == "update":
            ...
            if not os.path.isfile(db_path):
                ...
                sys.exit(1)
            # gisaid_ids of isolates - showing as unreleased in sqlite db & part of isolate_id group that has no released gisaid_ids - to be searched in gisaid
...
if __name__ == '__main__':
    ...
    update_parser = subparsers.add_parser('update', help='Update local SQLite db with isolate availability on GISAID EpiFlu.')
    download_parser = subparsers.add_parser('download', help = 'Download metadata or DNA/protein sequences from GISAID EpiFlu.')

    # subcommand arguments - upload
    upload_parser.add_argument('-i','--input', type=str, help='Path to folder containing "metadata" and "sequences" to upload.')
    add_common_args(upload_parser)
    upload_parser.add_argument('-c', '--clientid', type=str, help='GISAID EpiFlu clientID.')
    upload_parser.add_argument('-d', '--dateformat', type = str, choices = ["YYYYMMDD", "YYYYDDMM", "DDMMYYYY", "MMDDYYYY"],
    ...
    update_parser.add_argument('-b', '--database', type = str, help = 'Path to file to write sqlite database to (*.db).')
